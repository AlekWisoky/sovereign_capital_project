...
class BankrollManager:
    def __init__(self, cfg: BankrollConfig, *, state_path: str | None = None,
    ...
    def project_settled_outcome_state(self, economics: SettledReceiptEconomics) -> dict[str, Any]:
        ...
        if pnl > 0:
            ...
            payload["fail_streak"] = 0 if pnl == 0 else int(payload.get("fail_streak") or 0) + 1
            payload["success_streak"] = 0
        now_ms = self._now_ms()
        payload["updated_ts_ms"] = int(now_ms)
    ...
    def next_amount_in(self) -> int:
        ...
        # A signed loss must never be converted into reinvestable capital.
        # Cumulative negative P&L also suppresses compounding until recovery.
