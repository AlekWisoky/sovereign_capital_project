...
class BankrollManager:
    def __init__(self, cfg: BankrollConfig, *, state_path: str | None = None, history_repo: BankrollEventRepository | None = None, capital_event_repo: CapitalEventRepository | None = None):
        self.cfg = cfg
    ...
    def _default_state(self) -> BankrollState:
        now_ms = self._now_ms()
        return BankrollState(updated_ts_ms=now_ms, profit_updated_ts_ms=now_ms, sizing_updated_ts_ms=now_ms)
    ...
    def _load_state(self) -> BankrollState:
        ...
        try:
            recent_results = deque([int(v) for v in list(payload.get("recent_results") or [])], maxlen=window)
        except _SAFE_BANKROLL_STATE_IO_EXCEPTIONS:
            recent_results = deque(maxlen=window)
        try:
            recent_returns = deque([float(v) for v in list(payload.get("recent_returns") or [])], maxlen=window)
        except _SAFE_BANKROLL_STATE_IO_EXCEPTIONS:
    ...
    def _record_history(self, *, event_type: str, payload: dict[str, Any] | None = None) -> None:
        ...
        if self._history_repo is not None:
            try:
                self._history_repo.append_event(ts_ms=ts_ms, event_type=str(event_type or "unknown"), state=state_payload, payload=event_payload)
            except _SAFE_BANKROLL_STATE_IO_EXCEPTIONS:
                pass
        if self._capital_event_repo is None:
            return
        try:
            self._capital_event_repo.append_event(ts_ms=ts_ms, domain="bankroll", event_type=str(event_type or "unknown"), source="bankroll_manager", entity_id="bankroll_state", payload={"state": state_payload, "payload": event_payload})
        except _SAFE_BANKROLL_STATE_IO_EXCEPTIONS:
            return
    ...
    def _ensure_history_bootstrap(self) -> None:
        ...
        state_payload = self._state_payload()
        keys = ("realized_profit_wei", "realized_pnl_usd", "realized_loss_usd", "reinvestable_profit_usd", "last_settled_receipt_id", "last_settled_transaction_id", "last_settled_pnl_usd", "last_amount_in_wei", "success_streak", "fail_streak", "updated_ts_ms", "profit_updated_ts_ms", "sizing_updated_ts_ms")
    ...
    def project_trade_state(self, *, success: bool, realized_profit_after_gas_wei: int, amount_in_wei: int | None = None) -> dict[str, Any]:
        ...
        payload["profit_updated_ts_ms"] = int(now_ms) if bool(success) else int(payload.get("profit_updated_ts_ms") or now_ms)
        ...
        returns.append(float(max(0, int(realized_profit_after_gas_wei))) / float(amt_in) if bool(success) else 0.0)
        window = max(1, int(getattr(self.cfg, "kelly_window", 50) or 50))
    ...
    def project_settled_outcome_state(self, economics: SettledReceiptEconomics) -> dict[str, Any]:
        ...
        if economics.success and economics.realized_after_gas_wei > 0:
            payload["realized_profit_wei"] = int(payload.get("realized_profit_wei") or 0) + int(economics.realized_after_gas_wei)
        if pnl > 0:
            payload["success_streak"] = int(payload.get("success_streak") or 0) + 1
            payload["fail_streak"] = 0
        elif pnl < 0:
    ...
    def record_settled_outcome(self, economics: SettledReceiptEconomics) -> None:
        ...
        self._record_history(event_type="settled_outcome_recorded", payload={"receipt_id": economics.receipt_id, "transaction_id": economics.transaction_id, "signed_pnl_usd": economics.signed_pnl_usd, "loss_usd": economics.loss_usd, "reinvestable_profit_usd": economics.reinvestable_profit_usd, "source": economics.source})
    ...
    def apply_overrides(self, *, kelly_enabled: bool | None = None, auto_reinvest_enabled: bool | None = None) -> None:
        try:
            ...
            self._touch(); self._save_state()
            self._record_history(event_type="override", payload={"kelly_enabled": None if kelly_enabled is None else bool(kelly_enabled), "auto_reinvest_enabled": None if auto_reinvest_enabled is None else bool(auto_reinvest_enabled)})
        except _SAFE_BANKROLL_OVERRIDE_EXCEPTIONS:
            pass

    def record_trade(self, *, success: bool, realized_profit_after_gas_wei: int, amount_in_wei: int | None = None) -> None:
        state_payload = self.project_trade_state(success=bool(success), realized_profit_after_gas_wei=int(realized_profit_after_gas_wei), amount_in_wei=amount_in_wei)
        if bool(success):
            legacy_profit = float(max(0, int(realized_profit_after_gas_wei)))
            state_payload["realized_pnl_usd"] = float(state_payload.get("realized_pnl_usd") or 0.0) + legacy_profit
            state_payload["reinvestable_profit_usd"] = float(state_payload.get("reinvestable_profit_usd") or 0.0) + legacy_profit
        self.apply_state_payload(state_payload)
        self._record_history(event_type="trade_recorded", payload={"success": bool(success), "realized_profit_after_gas_wei": int(realized_profit_after_gas_wei), "amount_in_wei": int(state_payload.get("last_amount_in_wei") or 0)})
    ...
    def next_amount_in(self) -> int:
        ...
        if not self.cfg.auto_reinvest_enabled or self.cfg.reinvest_rate_pct <= 0 or not can_reinvest:
            amt = int(base)
        else:
            reinvest = (int(self.state.realized_profit_wei) * int(self.cfg.reinvest_rate_pct)) // 100
            amt = int(base + reinvest)
        if bool(self.cfg.kelly_enabled) and len(self.state.recent_returns) >= int(getattr(self.cfg, "kelly_min_history", 20) or 20):
        ...
        self._touch(sizing=True); self._save_state()
        self._record_history(event_type="sizing_decision", payload={"recommended_amount_in_wei": int(amt), "signed_realized_pnl_usd": float(self.state.realized_pnl_usd), "reinvestmentEligible": bool(can_reinvest)})
        return amt
    ...
    def _kelly_fraction(self) -> float:
        ...
        if var <= 1e-12:
            ...
            f = (mu / var) * (0.5 + 0.5 * p)
        f = max(0.0, f); f = f * (1.0 - max(0.0, min(0.80, vd)))
