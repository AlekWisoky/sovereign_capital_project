...
def _maybe_attach_omar(app: FastAPI, *, cfg_paths: List[str], boot_status: MutableMapping[str, Any]) -> None:
    ...
    except _OMAR_IMPORT_EXCEPTIONS as e:
    ...
    def get_omar_runtime():
        ...
        chain_name = str(((raw.get("chain") or {}) if isinstance(raw, dict) else {}).get("name") or "default")
    ...
    except _OMAR_ATTACH_EXCEPTIONS as e:
...
def create_app() -> FastAPI:
    ...
    app.add_middleware(CORSMiddleware, allow_origins=_cors_allow_origins(), allow_credentials=False, allow_methods=["*"], allow_headers=["*"], expose_headers=["*"])
