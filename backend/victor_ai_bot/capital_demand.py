...
class Money:
    ...
    def compatible_with(self, other: "Money") -> bool:
        return (self.asset, self.decimals, self.denomination) == (
            other.asset,
            other.decimals,
            other.denomination,
        )
...
class Capacity:
    ...
    def covers(self, execution_notional: Money, *, now: datetime) -> bool:
        return (
            self.valid(now=now)
            and self.unit == execution_notional.denomination
            and self.amount >= execution_notional.amount
        )
...
class ConversionEvidence:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            or not str(self.from_denomination).strip()
            or not str(self.to_denomination).strip()
        ):
    ...
    def convert(
        ...
        *,
        ...
        converted = (
            Decimal(amount.amount) * Decimal(self.numerator) / Decimal(self.denominator)
        ).to_integral_value(rounding=mode)
...
class CapitalDemand:
    ...
    def __post_init__(self) -> None:
        try:
            status = (
                self.status
                if isinstance(self.status, DemandStatus)
                else DemandStatus(str(self.status))
            )
        ...
        if (
            self.execution_notional.asset != self.execution_asset
            or self.execution_notional.decimals != self.execution_decimals
        ):
            raise CapitalDemandError("execution identity does not match execution notional")
        if (
            isinstance(self.treasury_decimals, bool)
            or not isinstance(self.treasury_decimals, int)
            or not 0 <= self.treasury_decimals <= 255
        ):
            raise CapitalDemandError("treasury decimals are invalid")
        treasury_values = (
            self.internal_capital_commitment,
            self.gas_reserve,
            self.fee_reserve,
            self.worst_case_exposure,
            self.strategy_budget_consumption,
        )
        if any(
            value.denomination != self.treasury_denomination
            or value.decimals != self.treasury_decimals
            for value in treasury_values
        ):
            raise CapitalDemandError(
                "treasury quantities must share declared treasury denomination and decimals"
            )
        if (
            self.max_worst_case_exposure is not None
            and not self.max_worst_case_exposure.compatible_with(self.worst_case_exposure)
        ):
    ...
    def is_flash_loan(self) -> bool:
        return self.capital_source == "flashloan" or self.strategy_family in {
            "flash_arb",
            "flashloan_atomic",
        }
    ...
    def validate(self, *, now: datetime) -> DemandStatus:
        ...
        if (
            ...
            or now < self.demand_generated_at
            or now > self.demand_expires_at
        ):
            return DemandStatus.STALE
        if (
            self.is_flash_loan
            and self.execution_notional.amount > 0
            and self.internal_capital_commitment.amount == 0
        ):
        ...
        if not self.provider_capacity_requirement.covers(self.execution_notional, now=now):
            return (
                DemandStatus.STALE
                if not self.provider_capacity_requirement.valid(now=now)
                else DemandStatus.INVALID
            )
        if (
            self.max_worst_case_exposure is not None
            and self.worst_case_exposure.amount > self.max_worst_case_exposure.amount
        ):
            return DemandStatus.INVALID
        values = [self.strategy_budget_consumption] + [
            source.strategy_budget_consumption for source in self.corroborating_sources
        ]
        if any(
            value.denomination != self.treasury_denomination
            or value.decimals != self.treasury_decimals
...
def require_same_treasury_denomination(budget: Money, demand: CapitalDemand) -> None:
    if (
        budget.denomination != demand.treasury_denomination
        or budget.decimals != demand.treasury_decimals
    ):
        raise CapitalDemandError("treasury budget and demand are not comparable")


def matches_execution_plan(
    demand: CapitalDemand, *, execution_plan_id: str, execution_notional: Money, now: datetime
) -> bool:
    return (
        demand.execution_plan_id == execution_plan_id
        and demand.execution_notional.compatible_with(execution_notional)
        and demand.execution_notional.amount == execution_notional.amount
        and demand.validate(now=now) == DemandStatus.VALID
    )


def apply_goal_cap(
    *, demand: CapitalDemand, cap: int, accepted: bool, now: datetime
) -> CapitalDemand:
...
def apply_aggressiveness_cap(
    *, demand: CapitalDemand, multiplier: int, safety_approved: bool, now: datetime
) -> CapitalDemand:
    ...
    return CapitalDemand(
        **{
            **demand.__dict__,
            "strategy_budget_consumption": Money(
                amount,
                demand.treasury_denomination,
                demand.treasury_decimals,
                demand.treasury_denomination,
            ),
        }
    )
...
def ceil_ratio(amount: int, numerator: int, denominator: int) -> int:
    if (
        not all(isinstance(value, int) and value >= 0 for value in (amount, numerator, denominator))
        or denominator == 0
    ):
        raise CapitalDemandError("invalid rounding inputs")
    return int(
        (Decimal(amount) * Decimal(numerator) / Decimal(denominator)).to_integral_value(
...
@dataclass(frozen=True)
class _CapitalDemandProjection:
    """Compatibility projection used by canonical settlement serialization."""
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "requested_usd_micro": self.requested_usd_micro,
            "authorized_usd_micro": self.authorized_usd_micro,
            "deployed_usd_micro": self.deployed_usd_micro,
            "authority_source": self.authority_source,
            "capital_source": self.capital_source,
            "goal_posture": self.goal_posture,
...
def capital_demand_from_mapping(value: Any) -> _CapitalDemandProjection:
    """Normalize capital-demand facts for the canonical settlement interface."""
    ...
    if not authority:
        authority = _safe_dict(root.get("capital_authority"))
    ...
    return _CapitalDemandProjection(
