...
class SettledReceiptEconomics:
    """Canonical economic facts derived from a settled receipt ledger record.

    This is intentionally independent of the receipt success boolean. A failed
    receipt can carry a negative economic result because gas, borrowing, or
    other settled costs were actually incurred.
    """
...
class MoneyLoopAccounting:
    """Translate the canonical settled ledger transaction into signed economics."""
    ...
    def from_settlement_payload(
        ...
        """Resolve signed P&L from the canonical receipt-settlement metadata.

        `net_realized_usd` is preferred because it is the value committed by the
        settlement writer. The component fields are retained as a deterministic
