...
def _coverage_summary(pyproject: dict, workflow_text: str) -> Dict[str, Any]:
    ...
    black_excludes = [
        line.strip()
        for line in str(black.get("extend-exclude") or "").splitlines()
        if line.strip().startswith("|^/backend") or line.strip().startswith("^/backend")
    ]
    ...
    return {
        ...
        "staged_expansion": {
            "fioa_shell_lint_domain_enabled": "backend/victor_ai_bot/fioa/__init__.py"
            in mypy_targets
            or "backend/victor_ai_bot/fioa/config.py" in mypy_targets,
            "execution_facade_type_targets": [
                x for x in mypy_targets if "runtime_execute_" in x or x.endswith("/execution.py")
            ],
        },
    }
...
def _critical_hardening_tests(backend_tests: List[str]) -> Dict[str, Any]:
    categories = {
        "runtime_compatibility": [
            x
            for x in backend_tests
            if "runtime_" in Path(x).name or "execution_service_auto_trade_hold" in x
        ],
        "execution_hardening": [x for x in backend_tests if "execution" in Path(x).name],
        "capital_and_fund_truth": [
            x
            for x in backend_tests
            if any(
                tok in Path(x).name for tok in ("ledger", "fund", "capital", "prime", "portfolio")
            )
        ],
    }
    return {
...
def build_verification_report() -> Dict[str, Any]:
    ...
    report = {
        ...
        "partially_verified": {
            ...
            "static_analysis_excluded_domains": _coverage_summary(pyproject, workflow_text)["ruff"][
                "excluded_domains"
            ],
        },
        "out_of_scope": [
...
def write_verification_report(out_dir: Path | None = None) -> Dict[str, Any]:
    ...
    (target_dir / "verification_report.json").write_text(
