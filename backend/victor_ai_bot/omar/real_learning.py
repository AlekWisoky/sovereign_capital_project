...
def _prime_context(context: Mapping[str, Any]) -> str:
    ...
    capacity_bucket = _bucket(
        capacity, (0.10, 0.50, 0.90), ("cap_low", "cap_mid", "cap_high", "cap_full")
    )
    cost_bucket = _bucket(
        cost_bps, (0.0, 5.0, 15.0), ("cost_free", "cost_low", "cost_mid", "cost_high")
    )
    return f"{source_bucket}:{availability}:{capacity_bucket}:{cost_bucket}"


def _intent_context(context: Mapping[str, Any]) -> str:
    """Stable operator-intent buckets; raw goal amounts and IDs never enter the key."""
    aggression = _text(context, "aggression_mode").lower() or "balanced"
    if aggression not in {"conservative", "balanced", "aggressive"}:
        aggression = "balanced"

    risk_multiplier = _clip(float(context.get("risk_multiplier") or 1.0), 0.10, 1.0)
    risk_bucket = _bucket(
        risk_multiplier,
        (0.50, 0.80, 0.95),
        ("risk_tight", "risk_reduced", "risk_near_full", "risk_full"),
    )

    horizon = float(context.get("goal_horizon_compatibility") or 1.0)
    ...
    ai_action = _text(context, "ai_recommendation_action").lower() or "none"
    ai_posture = _text(context, "ai_recommendation_posture").lower() or "none"
    confidence = _clip(float(context.get("ai_recommendation_confidence") or 0.0), 0.0, 1.0)
    confidence_bucket = _bucket(
        confidence, (0.50, 0.75, 0.90), ("ai_low", "ai_mid", "ai_high", "ai_very_high")
    )
    return (
        f"{aggression}:{risk_bucket}:{horizon_bucket}:{ai_action}:{ai_posture}:{confidence_bucket}"
    )
...
class OmarRealLearner:
    ...
    Correlation IDs and raw prime IDs remain lineage-only. Prime economics and
    operator-intent buckets are stable learning features so policy can generalize.
    """

    def __init__(
        self, *, path: str, alpha: float = 0.12, epsilon: float = 0.0, min_observations: int = 20
    ) -> None:
        self.path = path
    ...
    def state_key(context: Mapping[str, Any]) -> str:
        ...
        return "|".join(
            (
                ...
                _bucket(gas, (0.0002, 0.0005, 0.001), ("g_vlow", "g_low", "g_mid", "g_hi")),
                _bucket(p, (0.70, 0.80, 0.90), ("p_low", "p_mid", "p_high", "p_very_high")),
                _bucket(dd, (2.0, 5.0, 8.0), ("dd_low", "dd_mid", "dd_high", "dd_critical")),
                _bucket(realism, (0.55, 0.70, 0.85), ("r_low", "r_mid", "r_high", "r_strong")),
                _bucket(stability, (0.55, 0.70, 0.85), ("s_low", "s_mid", "s_high", "s_strong")),
                _bucket(
                    goal_gap,
                    (0.0, 2.0, 5.0),
                    ("goal_on_track", "goal_gap_small", "goal_gap_large", "goal_gap_extreme"),
                ),
                _bucket(vol, (0.10, 0.20, 0.35), ("v_low", "v_mid", "v_high", "v_extreme")),
        ...
        with self._lock:
            ...
            if action in {"WAIT", "DEFEND"}:
                ...
                rec = OmarRecommendation(
                    ...
                    True,
                    obs,
                    "learned_execution_action",
                )
            self.last_recommendation = rec.to_dict()
            return rec

    def observe(
        self, *, state_key: str, action: str, reward: float, outcome: Mapping[str, Any]
    ) -> Dict[str, Any]:
