...
class OmarTrainer:
    def __init__(self, cfg: OmarConfig, state_dim: int = 96, checkpoint_path: str | None = None):
        self.cfg = cfg
        ...
        self.policy = UnifiedRolePolicy(
            state_dim=self.state_dim, role_dim=cfg.role_vector_size, action_keys=DEFAULT_ACTION_KEYS, checkpoint_path=checkpoint_path
        )
        self._role_names = list(cfg.roles or [])
        self._role_embeds = {r: encode_role_vector(r, cfg.role_vector_size) for r in self._role_names}
        self.last_stats: OmarTrainStats | None = None
        self.last_real_learning: Dict[str, Any] = {"seen": 0, "learned": 0, "skipped": 0, "mean_reward_scaled": 0.0, "last_tx_hash": ""}
    ...
    def run_episode(self, episode: int) -> OmarTrainStats:
        ...
        adv = hierarchical_advantage(adv_turn, adv_token, self.cfg.turn_level_weight, self.cfg.token_level_weight)
        adv = (adv - adv.mean()) / (adv.std() + 1e-6)
        batch = {"X": np.stack(X_rows).astype(np.float32), "A": np.array(A_rows, dtype=np.int64), "ADV": adv.astype(np.float32), "OLD_P": np.array(OLD_P_rows, dtype=np.float32), "RET": (adv + values).astype(np.float32)}
        ppo = self.policy.update_ppo(batch, lr=self.cfg.learning_rate, clip_eps=self.cfg.clip_epsilon)
        stats = OmarTrainStats(episode=episode, mean_reward=float(np.mean(rewards)) if len(rewards) else 0.0, mean_coordination=float(np.mean(coord_hist)) if coord_hist else 0.0, mean_conflict=float(np.mean(conflict_hist)) if conflict_hist else 0.0, ppo=ppo)
        self.last_stats = stats
        return stats

    @staticmethod
    def _state_vector(rl_state: str, state_dim: int) -> np.ndarray:
        return encode_role_vector(f"OMAR_STATE:{str(rl_state or 'unknown')}", state_dim).astype(np.float32)
    ...
    def learn_from_real_outcomes(self, outcomes: Sequence[Any]) -> Dict[str, Any]:
        ...
        for outcome in list(outcomes or []):
            ...
            context = getattr(outcome, "context", {}) or {}
            brain = context.get("brain") if isinstance(context, dict) else {}
            role_name = str(brain.get("role") or "ARBITRAGE_AGENT") if isinstance(brain, dict) else "ARBITRAGE_AGENT"
            role_vec = self._role_embeds.get(role_name) or encode_role_vector(role_name, self.cfg.role_vector_size)
            stats = self.policy.update_from_real_outcome(role_vec=role_vec, state_vec=self._state_vector(rl_state, self.state_dim), action_index=action_index, reward_scaled=float(getattr(outcome, "reward_scaled_float", 0.0) or 0.0), learning_rate=float(self.cfg.learning_rate), clip_epsilon=float(self.cfg.clip_epsilon))
        ...
        if learned and self.cfg.policy_checkpoint_enabled:
            self.policy.save()
        self.last_real_learning = {"seen": seen, "learned": learned, "skipped": skipped, "mean_reward_scaled": float(np.mean(rewards)) if rewards else 0.0, "last_tx_hash": last_tx_hash, "policy_updates": int(self.policy.updates)}
