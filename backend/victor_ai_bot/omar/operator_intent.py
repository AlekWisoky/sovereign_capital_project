...
def snapshot_operator_intent(runtime: Any, opp: Any, decision: Any | None) -> tuple[dict[str, Any], str]:
    """Capture immutable-at-decision operator intent for learning attribution.

    Intent is contextual metadata only. Governance, capital admission, signing,
    and execution remain authoritative. A deep copy prevents later control
    mutations from rewriting the historical decision context.
    """
    raw_controls = getattr(getattr(runtime, "_cc", None), "controls", None)
    controls = _dict(raw_controls)
    if raw_controls is not None:
        for key in (
            "aggression_mode", "brain_mode", "defensive_mode", "control_mode",
            "auto_reinvest_enabled", "force_gas_mode", "force_send_mode", "risk_multiplier",
        ):
