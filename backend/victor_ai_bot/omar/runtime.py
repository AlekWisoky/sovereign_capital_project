...
class OmarRuntime:
    ...
    def _apply_real_learning_update(self, attribution: ActionAttribution) -> Dict[str, Any]:
        ...
        rl_state = str(state.get("rl_state") or state.get("state") or "")
        if not rl_state:
        ...
        if float(stats.get("updated", 0.0)) > 0.0 and trainer.cfg.policy_checkpoint_enabled:
            trainer.policy.save()
        return {
            **dict(stats),
            "updated": bool(float(stats.get("updated", 0.0)) > 0.0),
    ...
    def observe_decision(
        ...
        *,
        ...
        """Record canonical decision context for later settled attribution."""
    ...
    def observe_settled_ledger_record(self, row: Dict[str, Any]) -> Dict[str, Any]:
        """Consume exactly one canonical settled-ledger record.

        No learning occurs unless the canonical ledger bridge can resolve a
