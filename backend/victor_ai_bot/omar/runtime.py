...
from .real_learning import ACTIONS, OmarRealLearner, OmarRecommendation
from .trainer import OmarTrainer


class OmarRuntime:
    """Bounded OMAR learning runtime.

    The production learning boundary is explicit: decision snapshot -> canonical
    settled outcome -> integrity gate -> real learner. OMAR never signs,
    executes, approves capital, or bypasses governance/admission.
    """
    ...
    def __init__(self, cfg: OmarConfig, chain_name: str = "default"):
        ...
        self.learning_path = os.path.join(self.data_dir, "omar_learning", f"real_policy_{chain_name}.json")
    ...
    def observe_decision(
        ...
        *,
        ...
        row = {
            ...
            "canonical_lineage": {"decision_id": str(decision_id), "correlation_id": correlation_id},
    ...
    def observe_outcome(
        ...
        *,
        ...
        metadata_row = copy.deepcopy(dict(metadata or {}))
        settlement = metadata_row.get("settlement") if isinstance(metadata_row.get("settlement"), Mapping) else {}
        lineage = metadata_row.get("canonical_lineage") if isinstance(metadata_row.get("canonical_lineage"), Mapping) else {}
        outcome = dict(settlement)
        outcome.update({
            "status": str(settlement.get("status") or metadata_row.get("status") or "").strip(),
            "source": str(settlement.get("source") or metadata_row.get("source") or "").strip(),
            "decision_id": str(settlement.get("decision_id") or lineage.get("decision_id") or decision_id).strip(),
            "correlation_id": str(settlement.get("correlation_id") or lineage.get("correlation_id") or pending.get("correlation_id") or "").strip(),
            "opportunity_id": str(settlement.get("opportunity_id") or pending.get("opportunity_id") or "").strip(),
            "action": str(settlement.get("action") or pending.get("action") or "").strip(),
            "route_id": str(settlement.get("route_id") or route_id or pending.get("route_id") or "").strip(),
            "truth_verified": bool(settlement.get("truth_verified", settlement.get("outcome_truth_verified", outcome_truth_verified))),
            "canonical_lineage": {
                "decision_id": str(lineage.get("decision_id") or settlement.get("decision_id") or decision_id).strip(),
                "correlation_id": str(lineage.get("correlation_id") or settlement.get("correlation_id") or pending.get("correlation_id") or "").strip(),
            },
        })
        gate = validate_learning_transition(pending, outcome, decision_id=str(decision_id))
        if not gate.allowed:
            result = {"ok": False, "learned": False, "reason": gate.reason, "decision_id": str(decision_id)}
        ...
        self._log({"event": "omar_real_learning_update", **dict(result), "outcome": learner_outcome, "decision_snapshot": pending})
        return result
    ...
    def _loop(self) -> None:
        ...
        coord_hist: list[float] = []
        conflict_hist: list[float] = []
        cap_alloc = {role: 1.0 for role in (self.cfg.roles or [])}
        while not self._stop.is_set():
            if self._trainer and self._trainer.last_stats:
                stats = self._trainer.last_stats
                coord_hist.append(stats.mean_coordination)
                conflict_hist.append(stats.mean_conflict)
                coord_hist[:] = coord_hist[-200:]
                conflict_hist[:] = conflict_hist[-200:]
            metrics = compute_social_metrics(coord_hist=__import__("numpy").array(coord_hist, dtype=float), conflict_hist=__import__("numpy").array(conflict_hist, dtype=float), capital_alloc=cap_alloc)
            self.last_social = to_dict(metrics)
