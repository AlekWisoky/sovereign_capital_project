...
class OmarRuntime:
    ...
    def state(self) -> Dict[str, Any]:
        with self._lock:
            return {
                ...
                "cycle": self._cycle,
                "real_learning": self._real_learner.summary()
                if self._real_learner
                else {"enabled": False},
    ...
    def recommend(self, context: Mapping[str, Any]) -> OmarRecommendation:
        if (
            not self.enabled
            or not bool(getattr(self.cfg, "live_influence_enabled", True))
    ...
    def observe_decision(
        ...
        *,
        ...
        expected_net_usd: Any = None,
    ) -> None:
        if not self.enabled or not bool(getattr(self.cfg, "real_learning_enabled", True)):
            return
        metadata_row = copy.deepcopy(dict(metadata or {}))
        lineage = cast(dict[str, Any], metadata_row.get("canonical_lineage") or {})
        correlation_id = str(
            lineage.get("correlation_id") or metadata_row.get("correlation_id") or ""
        ).strip()
        sizing_id = str(
            lineage.get("sizing_id") or metadata_row.get("sizing_id") or ""
        ).strip()
        expected = (
            expected_net_usd
            if expected_net_usd is not None
            else metadata_row.get("expected_net_usd", context.get("expected_net_usd"))
        )
        canonical_lineage = {
    ...
    def observe_outcome(
        ...
        *,
        ...
        amount_in_wei: Any,
        gas_cost_usd: Any = None,
        slippage_bps: Any = None,
        latency_ms: Any = None,
        route_id: str = "",
        tx_hash: str = "",
        outcome_truth_verified: bool = False,
        metadata: Mapping[str, Any] | None = None,
    ) -> Dict[str, Any]:
        if (
            not self.enabled
        ...
        with self._lock:
            pending = copy.deepcopy(self._pending_decisions.get(str(decision_id), {}) or {})
        metadata_row = copy.deepcopy(dict(metadata or {}))
        settlement = cast(dict[str, Any], metadata_row.get("settlement") or {})
        lineage = cast(dict[str, Any], metadata_row.get("canonical_lineage") or {})
        pending_lineage = cast(dict[str, Any], pending.get("canonical_lineage") or {})
        outcome = dict(settlement)
        outcome.update(
            {
                "status": str(
                    settlement.get("status") or metadata_row.get("status") or ""
                ).strip(),
                "source": str(
                    settlement.get("source") or metadata_row.get("source") or ""
                ).strip(),
                "decision_id": str(
                    settlement.get("decision_id")
                    or lineage.get("decision_id")
                    or decision_id
                ).strip(),
                "correlation_id": str(
                    settlement.get("correlation_id")
                    or lineage.get("correlation_id")
                    or pending.get("correlation_id")
                    or ""
                ).strip(),
                "opportunity_id": str(
                ...
                "action": str(
                    settlement.get("action") or pending.get("action") or ""
                ).strip(),
                "route_id": str(
                    settlement.get("route_id") or route_id or pending.get("route_id") or ""
                ).strip(),
                "truth_verified": bool(
                    settlement.get(
                        "truth_verified", settlement.get("outcome_truth_verified", outcome_truth_verified)
                    )
                ),
                "settlement_verified": bool(
                    settlement.get(
                        "settlement_verified",
                        settlement.get("truth_verified", outcome_truth_verified),
                    )
                ),
                "execution_id": str(settlement.get("execution_id") or "").strip(),
                "outcome_id": str(settlement.get("outcome_id") or "").strip(),
                "sizing_id": str(
                    settlement.get("sizing_id")
                    or lineage.get("sizing_id")
                    or pending.get("sizing_id")
                    or pending_lineage.get("sizing_id")
                    or ""
                ).strip(),
                "canonical_lineage": {
                    "decision_id": str(
                        lineage.get("decision_id")
                        or settlement.get("decision_id")
                        or decision_id
                    ).strip(),
                    "correlation_id": str(
                        lineage.get("correlation_id")
                        or settlement.get("correlation_id")
                        or pending.get("correlation_id")
                        or ""
                    ).strip(),
                },
        ...
        if outcome["sizing_id"]:
            outcome["canonical_lineage"]["sizing_id"] = outcome["sizing_id"]
        outcome["expected_net_usd"] = (
            expected_net_usd
            if expected_net_usd is not None
            else settlement.get("expected_net_usd", pending.get("expected_net_usd"))
        )
        outcome["realized_net_usd"] = (
            realized_net_usd
            if realized_net_usd is not None
            else settlement.get("realized_net_usd")
        )
        ...
        if expected is not None and realized is not None:
            try:
                ...
                pass
        gate = validate_learning_transition(
            pending, outcome, decision_id=str(decision_id)
        )
        if not gate.allowed:
            result = {
                "ok": False,
            ...
            self._log({"event": "omar_learning_gate_rejected", **result})
            return result
        action = str(pending.get("action") or "")
        state_key = str(pending.get("state_key") or "")
        expected = float(pending["expected_net_usd"])
        realized = float(outcome["realized_net_usd"])
        expectation_error = realized - expected
        reward = (
            realized
            - 0.25 * max(0.0, -expectation_error)
            - max(0.0, float(outcome.get("slippage_bps") or 0.0)) * 0.01
            - max(0.0, float(outcome.get("latency_ms") or 0.0)) * 0.0001
        )
        if not ok:
            reward -= 1.0
        ...
        learner_outcome = {
            "decision_id": str(decision_id),
            "correlation_id": str(outcome.get("correlation_id") or ""),
            ...
            "gas_cost_usd": float(gas_cost_usd or 0.0),
        ...
        if outcome["sizing_id"]:
            learner_outcome["sizing_id"] = outcome["sizing_id"]
        result = self._real_learner.observe(
            state_key=state_key,
            action=action,
            reward=reward,
            outcome=learner_outcome,
        )
        result = {
            **dict(result),
            "learned": True,
        ...
        with self._lock:
            self._pending_decisions.pop(str(decision_id), None)
            self.last_outcome = {**dict(result), "action": action}
        self._log(
            {
    ...
    def _log(self, obj: Dict[str, Any]) -> None:
        try:
            with open(self.audit_path, "a", encoding="utf-8") as handle:
                handle.write(
                    json.dumps({**dict(obj), "ts": time.time()}, ensure_ascii=False) + "\n"
                )
    ...
    def _loop(self) -> None:
        if self.enabled and self.cfg.self_play_enabled:
            ...
            self.last_train = asdict(stats[-1]) if stats else {}
            self._log(
                {"event": "omar_training_complete", "last_train": self.last_train}
            )
