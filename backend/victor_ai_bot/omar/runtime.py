...
class OmarRuntime:
    ...
    def observe_decision(self, *, decision_id: str, opportunity_id: str, route_id: str, action: str, state_key: str, context: Mapping[str, Any], metadata: Mapping[str, Any] | None = None) -> None:
    ...
    def observe_outcome(self, *, decision_id: str, ok: bool, realized_net_usd: float, expected_net_usd: float, amount_in_wei: int, gas_cost_usd: float = 0.0, slippage_bps: float = 0.0, latency_ms: int = 0, route_id: str = "", tx_hash: str = "", outcome_truth_verified: bool = False, metadata: Mapping[str, Any] | None = None) -> Dict[str, Any]:
        ...
        reward = float(realized_net_usd) - 0.25 * max(0.0, float(expected_net_usd) - float(realized_net_usd)) - max(0.0, float(slippage_bps)) * 0.01 - max(0.0, float(latency_ms)) * 0.0001
        if not ok: reward -= 1.0
        reward = max(-50.0, min(50.0, reward))
        learner_outcome = {"decision_id": str(decision_id), "correlation_id": str(outcome.get("correlation_id") or ""), "route_id": str(outcome.get("route_id") or ""), "tx_hash": str(tx_hash), "ok": bool(ok), "realized_net_usd": float(realized_net_usd), "expected_net_usd": float(expected_net_usd), "amount_in_wei": int(amount_in_wei), "gas_cost_usd": float(gas_cost_usd), "slippage_bps": float(slippage_bps), "latency_ms": int(latency_ms), "outcome_truth_verified": True, "metadata": metadata_row}
        result = self._real_learner.observe(state_key=state_key, action=action, reward=reward, outcome=learner_outcome)
        with self._lock: self._pending_decisions.pop(str(decision_id), None); self.last_outcome = {**dict(result), "decision_id": str(decision_id), "action": action}
