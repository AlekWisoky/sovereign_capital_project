...
def validate_learning_transition(
    ...
    *,
    ...
    """Authorize OMAR policy learning only from a fully attributed settlement.

    This is deliberately a learning-only gate. It never approves capital,
    signs, executes, or changes governance/admission behavior.
    """
    pending_row = _dict(pending)
    row = _dict(outcome)
    did = _text(decision_id)
    correlation = _text(pending_row.get("correlation_id"))
    action = _text(pending_row.get("action"))
    ...
    if not _text(pending_row.get("state_key")):
        return LearningIntegrityResult(
            False, "missing_learning_state_key", did, correlation, action
        )
    if _text(row.get("status")).lower() != "settled":
        return LearningIntegrityResult(
            False, "outcome_not_canonically_settled", did, correlation, action
        )
    ...
    if not correlation or outcome_corr != correlation:
        return LearningIntegrityResult(
            False, "correlation_lineage_mismatch", did, correlation, action
        )

    outcome_action = _text(row.get("action"))
    if outcome_action and outcome_action != action:
        return LearningIntegrityResult(
            False, "action_attribution_mismatch", did, correlation, action
        )

    pending_opp = _text(pending_row.get("opportunity_id"))
    outcome_opp = _text(row.get("opportunity_id"))
    if pending_opp and outcome_opp and pending_opp != outcome_opp:
        return LearningIntegrityResult(
            False, "opportunity_lineage_mismatch", did, correlation, action
        )

    pending_route = _text(pending_row.get("route_id"))
    outcome_route = _text(row.get("route_id"))
    if pending_route and outcome_route and pending_route != outcome_route:
        return LearningIntegrityResult(
            False, "route_attribution_mismatch", did, correlation, action
        )
    ...
    if _text(row.get("source")) not in CANONICAL_SETTLEMENT_SOURCES:
        return LearningIntegrityResult(
            False, "noncanonical_learning_source", did, correlation, action
        )

    capital = _capital_context(pending_row)
    if _text(capital.get("capital_authority_source")) != "capital_engine_state":
        return LearningIntegrityResult(
            False, "capital_authority_not_canonical", did, correlation, action
        )
    if _text(capital.get("capital_authority_status")).lower() in {"", "unknown", "unavailable"}:
        return LearningIntegrityResult(
            False, "capital_authority_unavailable", did, correlation, action
        )
    if _text(capital.get("capital_authority_freshness")).lower() in {"", "unknown", "unavailable"}:
        return LearningIntegrityResult(
            False, "capital_authority_freshness_unknown", did, correlation, action
        )
