...
def _recommend_from_snapshot(
    learner: Any, context: Mapping[str, Any], snapshot: Mapping[str, Any], version: str
) -> OmarRecommendation:
    ...
    if not isinstance(values, Mapping):
        return OmarRecommendation(
            ...
            False,
            0,
            "promoted_policy_no_state",
        )
    ranked = sorted(
        ((str(action), float(value)) for action, value in values.items() if str(action) in ACTIONS),
    ...
    if not ranked:
        return OmarRecommendation(
            ...
            False,
            0,
            "promoted_policy_empty_state",
        )
    action, top = ranked[0]
    second = ranked[1][1] if len(ranked) > 1 else 0.0
    confidence = max(0.5, min(0.99, 0.5 + abs(top - second) * 0.05))
    observations = (
        int((snapshot.get("n") or {}).get(state_key, 0))
        if isinstance(snapshot.get("n"), Mapping)
        else 0
    )
...
def install_promotion_runtime_hooks() -> None:
    ...
    def recommend(self: Any, context: Mapping[str, Any]):
        ...
        if boundary.active_version == "baseline-v0":
            return OmarRecommendation(
                "", "UNTRAINED", 0.0, False, 1.0, "standard", False, 0, "no_promoted_policy"
            )
    ...
    def prepare_candidate(self: Any) -> str:
        ...
        boundary = _boundary(self)
        return boundary.register_candidate(
            _candidate_snapshot(learner),
            source_observations=int(getattr(learner, "total_observations", 0)),
        )

    def evaluate_candidate_oos(
        self: Any, candidate_version: str, events: list[Mapping[str, Any]]
    ) -> PromotionDecision:
