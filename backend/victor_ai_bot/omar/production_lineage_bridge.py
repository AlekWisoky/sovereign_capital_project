...
def _operator_intent_from_opportunity(opp: Any) -> dict[str, Any]:
    """Return the decision-time intent snapshot, if one was already captured."""
    meta = _dict(getattr(opp, "meta", None))
    lineage = _dict(meta.get("canonical_lineage"))
    brain = _dict(meta.get("brain"))
    for container in (lineage, brain):
        intent = container.get("operator_intent")
...
def _patch_execution_identity() -> None:
    ...
    async def wrapped(*args: Any, **kwargs: Any):
        ...
        if runtime is not None and opp is not None:
            try:
                # The decision boundary owns the immutable operator-intent snapshot.
                # Only legacy calls without that snapshot resolve live inputs.
                intent = _operator_intent_from_opportunity(opp)
                if not intent:
                ...
                if result is not None:
                    ...
                    plan["intent_fingerprint"] = intent_fingerprint(intent)
                    result.plan = plan
            except _SAFE:
                pass
