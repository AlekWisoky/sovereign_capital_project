...
def _canonical_lineage(pending: Mapping[str, Any]) -> dict[str, str]:
    ...
    decision_id = _text(
        meta.get("decision_id") or brain.get("canonical_decision_id") or brain.get("decision_id")
    )
...
def _patch_decision_lineage() -> None:
    ...
    if (
        original is None
        or getattr(original, "_canonical_identity_authoritative", False)
        or getattr(original, "_omar_lineage_patched", False)
    ):
        return
