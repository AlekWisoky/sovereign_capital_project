...
class UnifiedRolePolicy:
    ...
    The model remains dependency-free and intentionally lightweight. Its
    checkpoint is operational state: restarting OMAR must not erase learning.
    """
    ...
    def update_from_real_outcome(
        ...
        *,
        ...
        """Perform one bounded online update from a finalized real outcome."""
    ...
    def load(self) -> bool:
        ...
        try:
            ...
            if int(payload.get("version", 0)) != self.CHECKPOINT_VERSION:
                raise ValueError("unsupported_policy_checkpoint_version")
            if int(payload.get("state_dim", -1)) != self.state_dim:
                raise ValueError("policy_state_dim_mismatch")
            if int(payload.get("role_dim", -1)) != self.role_dim:
                raise ValueError("policy_role_dim_mismatch")
