...
def install_canonical_execution_invariant() -> None:
    ...
    for RuntimeBundle in bundles:
        ...
        if original_dispatch is not None and not getattr(
            original_dispatch, "_canonical_execution_guard", False
        ):

            def guarded_dispatch(
                self: Any, *, current_block: int, decision: Any = None, _original=original_dispatch
            ) -> bool:
        ...
        if original_execute is None or getattr(
            original_execute, "_canonical_execution_guard", False
        ):
            continue

        async def guarded_execute(
            self: Any, opp: Any, bn: int, decision: Any = None, _original=original_execute
        ) -> Any:
