...
_TREASURY_GOAL_ALLOWED_FIELDS = frozenset(
    {"target_return_percentage", "time_horizon_seconds", "risk_tolerance", "max_drawdown_pct", "capital_commitment_pct"}
)


def _treasury_unavailable(*, include_goal: bool = False) -> dict[str, object]:
    payload = unavailable_state("treasury_disabled", include_error=True, extra={"enabled": False})
    if include_goal:
...
def _treasury_goal_unavailable() -> dict[str, object]:
    return unavailable_state("treasury_goal_unavailable", include_error=True, extra={"goal": None, "enabled": False})


def _treasury_state_unavailable() -> dict[str, object]:
    return unavailable_state("treasury_state_unavailable", include_error=True, extra={"enabled": False})
...
def _treasury_capital_failed_payload() -> dict[str, object]:
    return with_auto_trade_route_projection(degraded_payload("treasury_capital_failed"))


def _treasury_state_failed_payload() -> dict[str, object]:
    return with_auto_trade_route_projection(degraded_payload("treasury_state_failed", extra={"enabled": False}))


def _treasury_goal_failed_payload() -> dict[str, object]:
    return with_auto_trade_route_projection(degraded_payload("treasury_goal_failed", extra={"goal": None, "enabled": False}))
...
def _invalid_set_goal_payload(payload: dict[str, object]) -> dict[str, object] | None:
    ...
    unknown_fields = unexpected_request_fields(payload, allowed_fields=_TREASURY_GOAL_ALLOWED_FIELDS)
    if unknown_fields:
        return invalid_request_payload("unknown_request_fields", details={"fields": unknown_fields})
    for field in ("target_return_percentage", "max_drawdown_pct", "capital_commitment_pct"):
        if field in payload and not coerce_non_negative_float(payload[field])[0]:
            return invalid_request_payload("invalid_float_value", field=field, value=payload[field])
    if "time_horizon_seconds" in payload and not coerce_positive_int(payload["time_horizon_seconds"])[0]:
        return invalid_request_payload("invalid_integer_value", field="time_horizon_seconds", value=payload["time_horizon_seconds"])
    if "risk_tolerance" in payload and not coerce_non_empty_string(payload["risk_tolerance"])[0]:
        return invalid_request_payload("invalid_string_value", field="risk_tolerance", value=payload["risk_tolerance"])
    return None
...
def _goal_read_payload(goal: object) -> dict[str, object]:
    ...
    return {"ok": True, "status": "available", "canonical": True, "service": "treasury_goal_route", "goal": resolve_goal_patch_payload({}, current_goal=goal_payload)}


def _treasury_has_goal(rt: object) -> bool:
    treasury = getattr(rt, "_treasury", None)
    return bool(treasury is not None and getattr(getattr(treasury, "cfg", None), "goal", None) is not None)
...
def _capital_projection(rt: object) -> dict[str, object]:
    ...
    if not isinstance(capital, dict) or capital.get("ok") is False:
        return _capital_engine_unavailable()
    treasury = AuxiliaryStateService().treasury_state(rt, capital_truth=AuxiliaryStateService().capital_truth(rt))
    if not isinstance(treasury, dict):
        return _treasury_capital_failed_payload()
    projection = dict(treasury)
    for key in ("equityUsd", "availableUsd", "settledUsd", "reservedUsd"):
        if key in capital:
            projection[key] = capital[key]
    return projection
...
def treasury_goal(rt=Depends(RuntimeBundle.dep)):
    return safe_json_route_call(
        lambda: with_auto_trade_route_projection(
            attach_summary_contract(
                _treasury_unavailable(include_goal=True) if getattr(rt, "_treasury", None) is None else _goal_read_payload(getattr(getattr(rt._treasury, "cfg", None), "goal", None)),
...
def set_treasury_goal(payload: dict[str, object], rt=Depends(RuntimeBundle.dep)):
    ...
    try:
        ...
        if "target_return_percentage" in payload:
            _, resolved["target_return_percentage"] = coerce_non_negative_float(payload["target_return_percentage"])
        if "time_horizon_seconds" in payload:
            _, resolved["time_horizon_seconds"] = coerce_positive_int(payload["time_horizon_seconds"])
        if "risk_tolerance" in payload:
            _, resolved["risk_tolerance"] = coerce_non_empty_string(payload["risk_tolerance"])
        if "max_drawdown_pct" in payload:
            _, resolved["max_drawdown_pct"] = coerce_non_negative_float(payload["max_drawdown_pct"])
        if "capital_commitment_pct" in payload:
            _, resolved["capital_commitment_pct"] = coerce_non_negative_float(payload["capital_commitment_pct"])
        ...
        return json_safe({"ok": False, "status": "unavailable", "error": "set_goal_failed", "reason_code": "set_goal_failed", "reason": "set_goal_failed"})
