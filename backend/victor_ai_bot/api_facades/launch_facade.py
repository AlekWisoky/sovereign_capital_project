...
_LAUNCH_CONTEXT_FAILURES = (AttributeError, KeyError, OSError, RuntimeError, TypeError, ValueError)
...
def guard_launch_mutation(*, runtime: Any, family: str, action: str) -> GovernanceOutcome:
    ...
    if rollout is None:
        return GovernanceOutcome(allowed=False, reason_code="launch_rollout_unavailable", review_required=False)
    if not family and action not in {"set_mode", "enable_next"}:
        return GovernanceOutcome(allowed=False, reason_code="family_required", review_required=False)
    ...
    # Explicit operator safety controls have precedence over advisory capital
    # navigation. A paused command center or quarantined family must report its
    # own deterministic blocker even when capital navigation is unavailable.
    ...
    if action == "enable_next":
        if controls is not None and bool(getattr(controls, "paused", False)):
            return GovernanceOutcome(allowed=False, reason_code="command_center_paused", review_required=False)
        if controls is not None and bool(getattr(controls, "allocations_frozen", False)):
            return GovernanceOutcome(allowed=False, reason_code="allocations_frozen", review_required=False)
        family_states = {
            canonical_launch_family_id(str(k or "")): v
            for k, v in (getattr(getattr(rollout, "profile", None), "family_states", {}) or {}).items()
        }
        if family and str(family_states.get(family) or "") == HealthState.QUARANTINED.value:
            return GovernanceOutcome(allowed=False, reason_code="family_quarantined", review_required=True)
