...
def ensure_decision_identity(
    ...
    *,
    ...
    Existing complete identity is reused; missing identity is created once and
    attached to both the decision and opportunity metadata. This helper does not
    authorize execution and does not fabricate execution/settlement IDs.
    """
    ...
    if isinstance(meta, dict):
        meta.setdefault("canonical_lineage", {})
        meta["canonical_lineage"].update(identity.to_dict())
        meta["canonical_lineage"].update(
            {
                "chain": _text(chain_name) or "default",
                "decision_block": int(current_block),
            }
        )
        if operator_intent is not None:
            ...
            meta["canonical_lineage"]["operator_intent"] = dict(payload)
        if intent_fingerprint:
            meta["intent_fingerprint"] = _text(intent_fingerprint)
            meta["canonical_lineage"]["intent_fingerprint"] = _text(intent_fingerprint)
