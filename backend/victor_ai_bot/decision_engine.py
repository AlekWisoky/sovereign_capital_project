...
class DecisionEngine:
        ...
        *,
        ...
        for o in opps[:50]:
            ...
            if not bool(getattr(o, "can_execute", False)) or not _exec_ready(o):
                self._set_brain(o, feats, 0.0, 0, action="skip", reason="not_executable")
                continue
            ...
            if borrow_mult > 1.0 and (p_base < 0.85 or feats.margin_ratio < 0.0010):
                borrow_mult = 1.0
        ...
        if smmae_mode:
            try:
                if self._smmae is None:
                    from victor_ai_bot.aqe import SMMAEConfig, SMMAEEngine
    ...
    def brain_state(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "aqe": aqe,
            "reward_trace": dict(self._last_reward_trace or {}),
            "route_stats": {"routes": int(len(self._route_stats))},
        }
    ...
    def _set_brain(
        ...
        *,
        ...
        bm = {
            ...
            "rl": {"state": rl_state, "action_index": int(rl_action)},
        }
        if gas_mode:
