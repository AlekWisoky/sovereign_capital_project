...
def choose_flashloan_size(
    *,
    ...
    capital_required = max(0.0, float(capital_required_usd or 0.0))
    institutional_requested_notional = INSTITUTIONAL_V1_POLICY.requested_notional_usd(capital_required)
    ...
    approved_notional = capital_required * float(best.get("borrow_mult") or 1.0) if capital_required > 0.0 else 0.0
    if capital_required > 0.0 and approved_notional < institutional_requested_notional:
        reason_codes.append("institutional_target_not_reached_by_current_safe_curve")
    ...
    return {
        ...
        "provider_limit": float(best.get("provider_limit") or _provider_limit(selected_provider_final)),
        ...
        "institutional_baseline_notional_usd": float(INSTITUTIONAL_V1_POLICY.baseline_target_usd),
        "institutional_requested_notional_usd": float(institutional_requested_notional),
        "institutional_approved_notional_usd": float(max(0.0, approved_notional)),
        "institutional_prime_capacity_target_usd": float(INSTITUTIONAL_V1_POLICY.internal_prime_capacity_target_usd),
    }
