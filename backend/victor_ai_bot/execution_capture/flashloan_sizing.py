...
def choose_flashloan_size(
    *, envelope: OpportunityEnvelope, requested_size_mult: float, route_plan: Dict[str, Any],
    ...
    aggressiveness: float | None = None, goal_gap_pct: float | None = None,
    max_borrow_usd: float | None = None, max_loss_usd: float | None = None,
    minimum_net_profit_usd: float = 0.0, minimum_net_roi_bps: float = 0.0,
    expected_loss_ratio: float = 0.0, max_size_mult: float | None = None,
) -> Dict[str, Any]:
    """Canonical flash-loan sizing entrypoint; adaptive Phase 23 is applied here."""
    legacy = _legacy_choose_flashloan_size(
        envelope=envelope, requested_size_mult=requested_size_mult, route_plan=route_plan,
        flashloan_resilience=flashloan_resilience, adversarial_state=adversarial_state,
        treasury_state=treasury_state, wealth_goal_state=wealth_goal_state,
    ...
    if isinstance(capital.get("capital_engine"), dict): capital = dict(capital["capital_engine"])
    ...
    max_borrow = float(max_borrow_usd if max_borrow_usd is not None else capital.get("max_borrow_usd", capital.get("maxBorrowUsd", capital.get("borrow_limit_usd", 0.0))))
    max_loss = float(max_loss_usd if max_loss_usd is not None else capital.get("max_loss_usd", capital.get("maxLossUsd", capital.get("loss_limit_usd", 0.0))))
    if max_borrow <= 0.0:
        max_borrow = max(0.0, float(capital.get("borrow_capacity_usd", capital.get("borrowCapacityUsd", 0.0))))
    if max_loss <= 0.0:
        max_loss = max(0.0, float(capital.get("risk_budget_usd", capital.get("riskBudgetUsd", 0.0))))
    if max_borrow <= 0.0 or max_loss <= 0.0:
        # Keep existing callers compatible; production callers should provide
        # authoritative capital_engine_state with explicit ceilings.
        max_borrow = max_borrow or 0.0
        max_loss = max_loss or 0.0
    if max_size_mult is None:
        max_size_mult = float(legacy.get("hard_cap") or 1.0)
    provider = str(legacy.get("selected_provider") or "aave")
    return apply_adaptive_flashloan_controller(
        legacy_result=legacy, canonical_decision_id=canonical_decision_id, correlation_id=correlation_id,
        route_id=str(envelope.route_id or route_plan.get("route_id") or ""), provider=provider,
        requested_size_mult=requested_size_mult, capital_engine_state=capital_engine_state,
        treasury_state=treasury_state, wealth_goal_state=wealth_goal_state, drawdown_state=drawdown_state,
        governance_allowed=governance_allowed, capital_authority_fresh=capital_authority_fresh,
        confidence=confidence, aggressiveness=aggressiveness_value, goal_gap_pct=goal_gap_value,
        max_borrow_usd=max_borrow, max_loss_usd=max_loss, minimum_net_profit_usd=minimum_net_profit_usd,
