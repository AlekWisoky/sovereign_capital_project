...
def normalize_human_context(source: Any) -> OmarHumanContext:
    ...
    return OmarHumanContext(
        ...
        ai_recommendation_id=(
            str(raw["ai_recommendation_id"]) if raw.get("ai_recommendation_id") else None
        ),
        ai_recommendation_source=(
            str(raw["ai_recommendation_source"]) if raw.get("ai_recommendation_source") else None
        ),
    )
...
def learning_features(
    ...
    aggressiveness = {"conservative": -1.0, "balanced": 0.0, "aggressive": 1.0}[
        context.aggressiveness_mode
    ]
    goal_progress = 0.0
    if context.desired_wealth_goal_amount and current_wealth is not None:
        goal_progress = max(
            -1.0, min(1.0, float(current_wealth) / context.desired_wealth_goal_amount - 1.0)
        )
    return {
