...
class RuntimeConstructorFacade:
    ...
    def _initialize_runtime_constructor_core(self, cfg: Any) -> None:
        ...
        self._lat = LatencyProfiler(
            window=int(os.environ.get("VICTOR_LAT_WINDOW", "400"))
        )
        ...
        self._opps = []
        self._errors = deque(
            maxlen=int(os.environ.get("VICTOR_ERROR_LOG_MAX", "200"))
        )
        self._ws_clients = []
        self._exec_log = deque(
            maxlen=int(os.environ.get("VICTOR_EXEC_LOG_MAX", "500"))
        )
        self._last_submitted_block = 0
        self._auto_trading = bool(cfg.execution.auto_trading)
        self._cb = CircuitBreaker.from_env()
        self._anomaly = AnomalyBreaker(
            window=int(os.environ.get("VICTOR_ANOM_WINDOW", "60"))
        )
        ...
        self._db = PersistenceDB(
            os.path.join(data_dir, "state", "xdv_runtime_state.sqlite3")
        )
        ...
        env_enabled = (
            os.environ.get("VICTOR_ENABLE_OMAR", "") or ""
        ).strip() == "1"
        configured = getattr(getattr(cfg, "superstructure", None), "omar", None)
        omar_cfg = (
            configured
            if isinstance(configured, OmarConfig)
            else OmarConfig(enabled=env_enabled)
        )
        if env_enabled:
        ...
        self._discovery = DiscoveryManager(
            chain_name=cfg.chain.name, data_dir=data_dir
        )
