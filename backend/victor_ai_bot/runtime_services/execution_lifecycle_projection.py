...
def build_receipt_summary_projection(receipt_summary: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "lastRuntimeFamily": str(receipt.get("lastRuntimeFamily") or last_identity.get("runtimeFamily") or ""),
        "lastCapitalFamily": str(receipt.get("lastCapitalFamily") or last_identity.get("capitalFamily") or ""),
        "lastDisplayFamily": str(receipt.get("lastDisplayFamily") or last_identity.get("displayName") or ""),
        "lastFamilyAliases": list(receipt.get("lastFamilyAliases") or last_identity.get("aliases") or []),
        ...
        "lastTerminalProfitabilityAuthority": _safe_dict(receipt.get("lastTerminalProfitabilityAuthority")),
...
def _build_execution_lifecycle(*, family_value: Any, live_items: List[Dict[str, Any]], receipt: Dict[str, Any], auto_trade_gate: Dict[str, Any], execution_gate: Dict[str, Any], v1_focus: Dict[str, Any]) -> Dict[str, Any]:
    family_info = family_identity_payload(family_value)
    family_launch = str(family_info.get("launchFamily") or "")
    family_items = [item for item in live_items if _lifecycle_matches_live_item(family_launch, item)]
    ...
    focus_aligned = bool(
        ...
        or family_matches(family=str(latest.get("runtimeFamily") or ""), candidate=v1_runtime_family)
    )
    can_trade_now = bool(auto_trade_gate.get("allowed", True)) and not bool(execution_gate.get("blocked", False))
    ...
    terminal_profitability = _safe_dict(effective_receipt.get("lastTerminalProfitabilityAuthority")) or _safe_dict(latest.get("terminalProfitabilityAuthority"))
    capital_admission = _safe_dict(effective_receipt.get("lastCapitalAdmission")) or _safe_dict(latest.get("capitalAdmission"))
    borrowing = _safe_dict(effective_receipt.get("lastBorrowing")) or _safe_dict(latest.get("flashloan"))
    loan_settlement = _safe_dict(effective_receipt.get("lastLoanSettlement"))
    provider = str(effective_receipt.get("lastProvider") or _safe_dict(latest.get("flashloan")).get("selectedProvider") or "")
    ...
    phase = str(_safe_dict(effective_receipt.get("stateContract")).get("phase") or ("execution_pending" if family_items else "idle"))
    return {
        ...
        "txHash": tx_hash,
        "provider": provider,
        "flashloanFeeWei": _safe_int(effective_receipt.get("lastFlashloanFeeWei") or borrowing.get("flashloanFeeWei")),
        "borrowCostUsd": _safe_float(effective_receipt.get("lastBorrowCostUsd") or borrowing.get("borrowCostUsd")),
        "borrowing": borrowing,
        ...
        "nextAction": str(closed_loop.get("nextAction") or auto_trade_gate.get("suggestedNextAction") or ""),
        "receiptSummaryState": _safe_dict(effective_receipt.get("stateContract")),
        "phase": phase,
    }


def build_execution_lifecycle_projections(*, live_execution: Dict[str, Any], receipt_summary: Dict[str, Any], auto_trade_gate: Dict[str, Any], execution_gate: Dict[str, Any], v1_focus: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    family_values = _unique_strings([
        ...
        *[str(_safe_dict(item).get("runtimeFamily") or _safe_dict(item).get("family") or _safe_dict(item).get("routeFamily") or "") for item in live_items],
    ])
    lifecycles = [_build_execution_lifecycle(family_value=family_value, live_items=live_items, receipt=receipt, auto_trade_gate=auto_trade_gate, execution_gate=execution_gate, v1_focus=v1_focus) for family_value in family_values]
    lifecycles.sort(key=lambda item: (0 if bool(item.get("v1FocusAligned")) else 1, 0 if bool(item.get("pendingActive")) else 1, 0 if bool(item.get("settlementRecorded")) else 1, str(item.get("family") or "")))
    return lifecycles


def select_focus_execution_lifecycle(lifecycles: List[Dict[str, Any]], v1_focus: Dict[str, Any]) -> Dict[str, Any]:
    focus_family = str(v1_focus.get("launchFamily") or "")
    for item in lifecycles:
        if family_matches(family=str(item.get("family") or ""), candidate=focus_family):
            return dict(item)
    return _build_execution_lifecycle(family_value=v1_focus.get("runtimeFamily") or DEFAULT_FAMILY, live_items=[], receipt={}, auto_trade_gate={"allowed": False, "stage": "unavailable", "reasonCodes": ["focus_lifecycle_unavailable"]}, execution_gate={"blocked": True, "reason_codes": ["focus_lifecycle_unavailable"]}, v1_focus=v1_focus)
