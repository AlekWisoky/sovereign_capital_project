...
class CanonicalSettledOutcomeLineage:
    ...
    The record is intentionally read/attribution oriented. It never authorizes
    execution or capital. Latency is retained as a realized delivery feature so
    OMAR can learn from execution quality without making latency a governance
    bypass or an implicit execution authorization.
    """
...
def resolve_settled_lineage(row: Mapping[str, Any]) -> CanonicalSettledOutcomeLineage:
    """Resolve the complete decision -> execution -> settlement chain.

    Supports both the canonical nested ``lineage`` envelope and the existing
    flat/metadata fields used by the treasury transaction journal. Missing
    identity is reported as an incomplete lineage rather than fabricated.
    """
