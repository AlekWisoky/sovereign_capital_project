...
def resolve_settled_lineage(row: Mapping[str, Any]) -> CanonicalSettledOutcomeLineage:
    ...
    decision_id = _first(nested.get("decision_id"), nested.get("decisionId"), source.get("decision_id"), source.get("decisionId"), metadata.get("decision_id"), metadata.get("decisionId"), decision.get("decision_id"), decision.get("decisionId"))
    correlation_id = _first(nested.get("correlation_id"), nested.get("correlationId"), source.get("correlation_id"), source.get("correlationId"), metadata.get("correlation_id"), metadata.get("correlationId"), decision.get("correlation_id"), decision.get("correlationId"), execution.get("correlation_id"), execution.get("correlationId"))
    execution_id = _first(nested.get("execution_id"), nested.get("executionId"), source.get("execution_id"), source.get("executionId"), metadata.get("execution_id"), metadata.get("executionId"), execution.get("execution_id"), execution.get("executionId"))
    settlement_id = _first(nested.get("settlement_id"), nested.get("settlementId"), source.get("settlement_id"), source.get("settlementId"), metadata.get("settlement_id"), metadata.get("settlementId"), outcome.get("settlement_id"), outcome.get("settlementId"))
    latency = _latency_ms(source, metadata)
    reasons = [f"missing_{field_name}" for field_name in _REQUIRED if not _text(locals()[field_name])]
    ...
    return CanonicalSettledOutcomeLineage(
        ...
        opportunity_id=_first(nested.get("opportunity_id"), nested.get("opportunityId"), source.get("opportunity_id"), source.get("opportunityId"), metadata.get("opportunity_id"), metadata.get("opportunityId")),
        ...
        latency_ms=latency, latency_class=latency_class(latency), complete=not reasons,
        reason_codes=reasons, metadata={**metadata, "lineageResolved": True},
    )
