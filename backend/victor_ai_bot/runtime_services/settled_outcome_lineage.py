...
def _economic_fields(source: Mapping[str, Any], metadata: Mapping[str, Any]) -> Dict[str, float]:
    ...
    if realized_net == 0.0 and realized_gross != 0.0:
        realized_net = realized_gross - _float(
            _first(
    ...
    return {
        ...
        "expected_latency_ms": _float(
            _first(
                expected.get("latency_ms"),
                metadata.get("expected_latency_ms"),
            ),
            0.0,
        ),
...
def resolve_settled_lineage(row: Mapping[str, Any]) -> CanonicalSettledOutcomeLineage:
    ...
    try:
        resolved_economics = resolve_net_economics(
            type(
                "Outcome",
                (),
                {
                    ...
                    "realized_gas_cost_usd_micro": _float(
                        source.get("realized_gas_cost_usd_micro"), 0.0
                    ),
                    "realized_profit_after_gas_usd_micro": _float(
                        source.get("realized_profit_after_gas_usd_micro"), 0.0
                    ),
                    "realized_profit_usd_micro": _float(
                        source.get("realized_profit_usd_micro"), 0.0
                    ),
        ...
        if resolved_economics.complete:
            ...
            if economics["realized_costs_usd"] == 0.0:
                economics["realized_costs_usd"] = max(
                    0.0, economics["realized_gross_usd"] - economics["realized_net_usd"]
                )
