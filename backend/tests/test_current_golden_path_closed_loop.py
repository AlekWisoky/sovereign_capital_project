...
from victor_ai_bot.ethabi import enc_uint
...
from victor_ai_bot.models import Metrics, Opportunity
...
class _Runtime:
    def __init__(self, tmp_path):
        self.cfg = SimpleNamespace(
            chain=SimpleNamespace(
                name="ethereum", chain_id=1, v3_pairs=[], curve_pools=[], balancer_pools=[],
            ...
            safety=SimpleNamespace(slippage_bps=50, minProfitAbs=1, minProfitBps=0, flashloan_fee_bps=9, max_borrow_amount="0", require_estimate_gas=False, require_simulation=False, mev_adversarial_eval_enabled=False, max_daily_loss_pct=3.0),
            execution=SimpleNamespace(dry_run=True, brain_mode="auto", gas_mode="standard", send_mode="public", gas_presets={}, gas_limit=21_000, max_submit_per_block=1, flashloan_fee_bps=9, flash_provider="aave", deadline_seconds=30, executor_address="0x00000000000000000000000000000000000000e4", profit_to="", private_key_env="UNUSED", rft=SimpleNamespace(enabled=True, episode_export_enabled=True, snapshot_top_k=3)),
        )
        ...
        self._bankroll = BankrollManager(BankrollConfig(auto_reinvest_enabled=True, base_borrow_amount_wei=1_000), history_repo=self._bankroll_history_repo, capital_event_repo=self._capital_event_repo)
        self._treasury = TreasuryRuntime(cfg=TreasuryConfig(enabled=True), data_dir=str(tmp_path), db=self._db, chain="ethereum", capital_event_repo=self._capital_event_repo)
        self._market_regime = {"regime": "balanced"}
        from victor_ai_bot.runtime_subsystems.replay_store import ReplayBundleStore
        self._replay = ReplayBundleStore(data_dir=str(tmp_path), chain="ethereum", chain_id=1)
        self._lat = LatencyProfiler()
        self._receipt_summary = {}
        self._receipt_service = SimpleNamespace(summarize=lambda runtime: dict(self._receipt_summary))
    ...
    async def snapshot(self):
        return {"chain": "ethereum", "metrics": self.metrics.model_dump(), "rpc": {"error_rate": 0.0}, "opportunities": []}
...
async def test_current_golden_path_closed_loop(tmp_path, monkeypatch):
    ...
    executor = runtime.cfg.execution.executor_address
    runtime.cfg.chain.v3_pairs = [{"token_in": token_a, "token_out": token_b, "fee": 3_000}]

    async def fixed_quotes(rpc, cfg, cache, edges, amount):
        return {arb_engine.edge_key(edge): (1_100 if edge.token_in == token_a and amount == amount_in else 1_050, {"fee": 3_000}) for edge in edges}

    monkeypatch.setattr(arb_engine, "quote_edges_batch", fixed_quotes)
    opportunities = await RuntimePrimaryScanFacade._scan_primary_opportunities(runtime, _QuoteRpc(), current_block=19_000_000, amount_in=amount_in)
    ...
    # SYNTHETIC_ENRICHMENT: the scanner intentionally emits can_execute=False and
    # does not create capture/profitability/admission metadata. These fields are
    # injected only to exercise the existing downstream contracts; this does not
    # prove the production runtime generates them on this isolated call.
    opp.can_execute = True
    opp.meta.update({
        "strategy_family": "flashloan_atomic", "route_family": "flashloan_atomic",
    ...
    decision = DecisionEngine(chain_name="ethereum", data_dir=str(tmp_path), brain_mode="auto").annotate_and_decide([opp], current_block=19_000_000, pending_txs=0, auto_enabled=True, cfg=runtime.cfg, gas_budget_remaining_wei=10**12, capital_budget_remaining_wei=10**12, family_capital_remaining_wei={"flashloan_atomic": 10**12})
    assert decision.action == "trade"
    assert decision.opp_id == opp.id
    assert decision.route_id == opp.route_id

    admission_probe = _AdmissionProbe()
    admission = admission_probe.auto_trade_admission_gate(runtime, opp, decision)
    assert admission.allowed is True
    assert admission_probe.calls == ["hold", "family", "route", "flashloan", "treasury"]

    async def fixed_gas(rpc, mode=None, presets=None):
        return 1, 1

    monkeypatch.setattr(execution, "suggest_gas", fixed_gas)
    dry_result = await try_execute_opportunity(_NoSendRpc(), _NoSendRpc(), runtime.cfg, opp, 19_000_000, 18_999_999, cache=None, decision=decision, force_dry_run=True, profiler=None)
    assert dry_result.ok is True and dry_result.dry_run is True
    ...
    # expected row, and receipt below model a hypothetical submitted execution.
    tx_hash = "0x" + "ab" * 32
    await runtime._pnl.add_trade({"ts": 1, "chain": "ethereum", "opportunity_id": opp.id, "route_id": opp.route_id, "tx_hash": tx_hash, "mode": "auto", "dry_run": False, "ok": True, "reason": "submitted", "expected_gross_profit_wei": "50", "expected_profit_after_costs_wei": "25", "estimated_gas_cost_wei": "21000", "flashloan_fee_wei": "0", "gas_limit": 21000, "max_fee_wei": "1", "priority_fee_wei": "1", "strategy_type": "flashloan_atomic", "income_stream": "arb", "venue_path": "fixed"})
    ...
    log = {"address": executor, "topics": [ARB_EXECUTED_TOPIC0, route_topic, token_topic], "data": "0x" + (enc_uint(1_000) + enc_uint(50) + enc_uint(1)).hex()}
    receipt = {"status": "0x1", "gasUsed": "0x5208", "effectiveGasPrice": "0x1", "blockNumber": "0x122", "logs": [log]}
    decoded = decode_arb_executed(log)
    assert decoded is not None and decoded.profit == 50 and decoded.provider == 1

    pnl = await runtime._pnl.update_receipt(tx_hash, receipt, executor_address=executor, chain_weth=token_a)
    assert pnl["status"] == 1
    assert pnl["realized_profit_token_wei"] == "50"
    assert pnl["realized_gas_cost_wei"] == "21"

    settled = ReceiptService().synchronize_settlement_accounting(runtime, tx_hash=tx_hash, pending={"strategy_family": "flashloan_atomic", "route_family": "flashloan_atomic", "flashloan_fee_wei": "0"}, decoded={"realized_profit_after_gas_wei": "50", "realized_profit_token": token_a, "realized_profit_token_wei": "50", "realized_profit_after_gas_usd_micro": "50", "realized_gas_cost_wei": "21", "realized_gas_cost_usd_micro": "0"}, status=1, amount_in=amount_in, expected_after=25, realized_after=50, submit_to_receipt_ms=10, route_id=opp.route_id, route_family="flashloan_atomic", strategy_family="flashloan_atomic", capture_lane_pending="PUBLIC")
    ...
    capital_events = runtime._capital_event_repo.tail(limit=10)
    ...
    runtime._receipt_summary = {"ok": True, "lastTxHash": tx_hash, "lastRouteFamily": "flashloan_atomic", "lastFamily": "flashloan_atomic", "lastRuntimeFamily": "flashloan_atomic", "lastCapitalFamily": "flashloan_atomic", "lastDisplayFamily": "Flash Arbitrage", "lastProvider": "aave", "lastClosedLoop": {"settlementAccounting": True, "completed": True}}
    ...
    assert bundle["tx_hash"] == tx_hash and bundle["opportunity_id"] == opp.id
