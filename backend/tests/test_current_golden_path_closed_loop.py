...
class _Runtime:
    def __init__(self, tmp_path):
        self.cfg = SimpleNamespace(
            ...
            flags=SimpleNamespace(
                enable_two_leg_loops=True, enable_three_leg_loops=False, enable_v3_triangular=False
            ),
            ...
            execution=SimpleNamespace(
                ...
                flash_provider="aave",
                deadline_seconds=30,
                executor_address="0x00000000000000000000000000000000000000e4",
                profit_to="",
        ...
        self._cc = SimpleNamespace(
            controls=SimpleNamespace(defensive_mode=False),
            audit=AuditStore(str(tmp_path / "audit.jsonl")),
        )
        ...
        self._ledger_repo = LedgerRepository(
            self._db, capital_event_repo=self._capital_event_repo, chain="ethereum"
        )
        ...
        self._receipt_service = SimpleNamespace(
            summarize=lambda runtime: dict(self._receipt_summary)
        )
    ...
    def capital_truth(self):
        return SimpleNamespace(
            capital_summary={"deployableUsd": 100.0, "navUsd": 100.0, "utilizationPct": 0.0}
        )
...
async def test_current_golden_path_closed_loop(tmp_path, monkeypatch):
    ...
    async def fixed_quotes(rpc, cfg, cache, edges, amount):
        return {
            arb_engine.edge_key(edge): (
                1_100 if edge.token_in == token_a and amount == amount_in else 1_050,
                {"fee": 3_000},
            )
            for edge in edges
        }
            ...
            "capture": {
                ...
                "metadata": {
                    ...
                    "flashloan_resilience": {
                        "sizing": {"allowed": True, "selected_provider": "aave"}
                    },
                },
            },
            "out1": "1100",
    ...
    decision = DecisionEngine(
        chain_name="ethereum", data_dir=str(tmp_path), brain_mode="auto"
    ).annotate_and_decide(
        [opp],
        ...
        auto_enabled=True,
        cfg=runtime.cfg,
        gas_budget_remaining_wei=10**12,
        capital_budget_remaining_wei=10**12,
    ...
    await runtime._pnl.add_trade(
        {
            ...
            "ok": True,
            "reason": "submitted",
            "expected_gross_profit_wei": "50",
            "expected_profit_after_costs_wei": "25",
            "estimated_gas_cost_wei": "21000",
            "flashloan_fee_wei": "0",
            "gas_limit": 21000,
            "max_fee_wei": "1",
            "priority_fee_wei": "1",
            "strategy_type": "flashloan_atomic",
            "income_stream": "arb",
            "venue_path": "fixed",
        }
    )
    route_topic = (
        "0x" + opp.route_id[2:].rjust(64, "0")
        if opp.route_id.startswith("0x")
        else "0x" + opp.route_id.rjust(64, "0")
    )
    token_topic = "0x" + token_a[2:].rjust(64, "0")
    log = {
        "address": executor,
        "topics": [ARB_EXECUTED_TOPIC0, route_topic, token_topic],
        "data": "0x" + (enc_uint(1_000) + enc_uint(50) + enc_uint(1)).hex(),
    }
    receipt = {
        "status": "0x1",
        "gasUsed": "0x5208",
    ...
    pnl = await runtime._pnl.update_receipt(
        tx_hash, receipt, executor_address=executor, chain_weth=token_a
    )
    assert pnl["status"] == 1
    ...
    settled = ReceiptService().synchronize_settlement_accounting(
        ...
        decoded={
            "realized_profit_after_gas_wei": "50",
            "realized_profit_token": token_a,
            "realized_profit_token_wei": "50",
            "realized_profit_after_gas_usd_micro": "50",
            "realized_gas_cost_wei": "21",
            "realized_gas_cost_usd_micro": "0",
        },
        status=1,
        amount_in=amount_in,
        expected_after=25,
        ...
        route_family="flashloan_atomic",
        strategy_family="flashloan_atomic",
        capture_lane_pending="PUBLIC",
    )
    assert settled["ok"] is True
    assert runtime._ledger_repo.has_receipt_transaction(
        chain="ethereum", receipt_id=tx_hash, tx_type="receipt_settlement"
    )
    ...
    commit_ids = {
        str((event.get("payload") or {}).get("capitalCommitId"))
        for event in capital_events
        if (event.get("payload") or {}).get("capitalCommitId")
    }
    assert len(commit_ids) == 1

    runtime._receipt_summary = {
        "ok": True,
        "lastTxHash": tx_hash,
        "lastRouteFamily": "flashloan_atomic",
        "lastFamily": "flashloan_atomic",
        "lastRuntimeFamily": "flashloan_atomic",
        "lastCapitalFamily": "flashloan_atomic",
        "lastDisplayFamily": "Flash Arbitrage",
        "lastProvider": "aave",
        "lastClosedLoop": {"settlementAccounting": True, "completed": True},
    }
    replay = ReplayService()
    event_id = replay.create_bundle(
        runtime,
        opportunity_id=opp.id,
        route_id=opp.route_id,
        mode="auto",
        rl_state=decision.rl_state,
        rl_action=decision.rl_action_index,
        latency_ms=10,
