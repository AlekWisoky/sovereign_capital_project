...
from victor_ai_bot.capital_demand import (
    CapitalDemandError,
    Money,
    project_strategy_budget,
    require_same_treasury_denomination,
    selector_scalar,
)
...
def test_projection_is_only_strategy_budget_consumption():
    value = demand(
        strategy_budget_consumption=Money(17, "USD", 2, "USD"),
        execution_notional=Money(99_999_999, "USDC", 6, "USDC"),
    )
...
def test_projection_rejects_unknown_stale_conflicting_or_zero_demand():
    ...
    with pytest.raises(CapitalDemandError):
        selector_scalar(
            demand(strategy_budget_consumption=Money(0, "USD", 2, "USD")),
            now=NOW,
        )
