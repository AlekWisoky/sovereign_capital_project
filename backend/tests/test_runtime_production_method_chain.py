...
@pytest.mark.asyncio
async def test_actual_runtime_method_chain_reaches_production_execution_boundary(monkeypatch):
    """Walk the real RuntimeBundle orchestration methods, stubbing only leaf I/O.

    This deliberately does not replace the production orchestration facades:
    ...
    External market/RPC/execution leaves are stubbed so the test is deterministic
    and cannot broadcast a transaction.
    """
    ...
    runtime._stop = SimpleNamespace(is_set=lambda: False)
    ...
    runtime.cfg = SimpleNamespace(chain=SimpleNamespace(chain_id=1))
    ...
    runtime._pending = {}
    runtime._opps = [SimpleNamespace(id="opp-1")]
    ...
    def fake_dispatch(*, current_block, decision):
        events.append("auto_dispatch")
        return True
    ...
    # Keep the production orchestration methods intact; patch only leaf calls.
    ...
    runtime._maybe_dispatch_auto_trade = fake_dispatch
    ...
    assert runtime._opps == [SimpleNamespace(id="opp-1")] or runtime._opps
