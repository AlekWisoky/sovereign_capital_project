...
async def test_operator_intent_is_immutable_per_decision_through_execution_boundary():
    """Changing operator inputs between decisions cannot rewrite prior attribution."""
    ...
    runtime._cc = SimpleNamespace(controls=_Controls("conservative", 0.40))
    ...
    await original(runtime, opp1, result, bn=101, latency_ms=5, mode="auto")
    ...
    assert second["goal"]["timeframe_days"] == 90
