...
def test_actual_receipt_finalize_carries_pending_identity_to_settlement_and_persistence(
    ...
    def finalize_replay(**kwargs):
        calls.append(("replay", kwargs))

    def observe_outcome_truth_health(**kwargs):
        calls.append(("truth_health", kwargs))
    ...
    def synchronize_settlement_accounting(**kwargs):
        ...
        return {
            ...
            "metadata": {"canonical_lineage": dict(pending.get("canonical_lineage") or {})},
        }

    def update_execution_learning(**kwargs):
        calls.append(("learning", dict(kwargs["pending"])))

    def observe_settlement_memory(**kwargs):
        calls.append(("memory", dict(kwargs["pending"])))

    def update_agent_performance(**kwargs):
        calls.append(("performance", dict(kwargs["pending"])))

    service.finalize_replay = finalize_replay
    service.observe_outcome_truth_health = observe_outcome_truth_health
    ...
    service.update_execution_learning = update_execution_learning
    service.observe_settlement_memory = observe_settlement_memory
    service.update_agent_performance = update_agent_performance
