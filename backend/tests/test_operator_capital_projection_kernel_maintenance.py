...
class _Runtime:
    def __init__(self):
        self.cfg = SimpleNamespace(
            execution=SimpleNamespace(auto_trading=True, base_borrow_amount="0", gas_mode="fast", send_mode="private", v1_focus="flashloan_atomic"),
            chain=SimpleNamespace(name="ethereum", v3_pairs=[{"amount_in": "150"}], curve_pools=[], balancer_pools=[]),
            safety=SimpleNamespace(max_daily_loss_pct=3.0),
        )
        self.metrics = SimpleNamespace(exec_e2e_p50_ms=10, exec_e2e_p90_ms=20, exec_e2e_p99_ms=40, submit_to_receipt_p50_ms=30, submit_to_receipt_p90_ms=50, submit_to_receipt_p99_ms=90, loop_p50_ms=5, loop_p90_ms=10, loop_p99_ms=20, gas_mode="fast", send_mode="private", last_tick_ms=7)
        self._cc = SimpleNamespace(
            controls=SimpleNamespace(paused=False, sandbox_only=False, defensive_mode=False, control_mode="auto", governance_enabled=True, mutation_enabled=False, evolution_frozen=True, allocations_frozen=False, metrics_enabled=True, latency_profiling_enabled=True, reward_trace_enabled=True, chaos_breakers_enabled=True, rpc_batch_enabled=False, rft_episode_export_enabled=False, kelly_enabled=True, auto_reinvest_enabled=False, force_send_mode="", force_gas_mode="", brain_mode="", aggression_mode="balanced", full_system_enabled=False),
            audit=SimpleNamespace(tail=lambda limit=250: []), state=lambda: {"degraded": False},
        )
        ...
        self._ledger = SimpleNamespace(
            tail=lambda limit=50: [{"asset": "USD", "amount": 12.5, "receipt_id": "0xabc", "transaction_id": "tx123"}],
            transactions_tail=lambda limit=50: [{"tx_type": "prime_loan_open", "transaction_id": "tx123", "receipt_id": "0xabc"}],
    ...
    async def _pnl_summary(self, window=50):
        return {"total_realized_profit_after_gas_usd": 9.25, "realized_profit_after_gas_usd_micro": "9250000", "recent": []}
    ...
    def capital_summary(self):
        return {"ok": True, "navUsd": 12.5, "navSource": "ledger_usd_balance", "deployableUsd": 10.0, "estimatedCapitalUsd": 20.0, "deployedCapitalUsd": 4.0, "utilizationPct": 25.0, "exposure": {"activePct": 20.0, "sandboxPct": 5.0, "idlePct": 65.0, "atRiskPct": 10.0}, "internalPrime": {"borrowedUsd": 2.0, "utilization": 0.1}, "ledger": {"balances": {"USD": 12.5}, "transactions": [{"transaction_id": "tx123"}], "tail": [{"receipt_id": "0xabc"}], "accounting": {"assetAccounts": {"USD": 12.5}}}, "lastSettlement": {"receiptId": "0xabc", "transactionId": "tx123", "status": "settled"}, "terminalProfitabilityAuthority": {"stage": "realized_settlement", "authoritative": True}, "capitalAdmission": {"ok": True, "stateContract": {"phase": "capital_admission"}}, "familyCapitalPlanVersion": "family_capital_plan_v1", "familyCapitalPlan": [{"id": "flashloan_atomic", "launchFamily": "flash_arb"}], "allocations": [], "capitalFlows": []}
