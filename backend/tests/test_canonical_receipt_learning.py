...
def test_learning_receives_signed_settled_pnl(monkeypatch):
    captured = {}
    ...
    CanonicalReceiptService().update_decision_learning(
        runtime, route_id="route-1", rl_state="state-1", rl_action=2, amount_in=1000,
        expected_after=50, realized_after=0, status=0, tx_hash="receipt-1", mode="auto",
        latency_ms=4, submit_to_receipt_ms=20, aqe_action="EXECUTE", pending={"opportunity_id": "opp-1"},
