...
from victor_ai_bot.runtime_services.capital_truth_service_shell import (
    build_capital_truth_service_summary,
    summarize_capital_truth,
)
...
class _BankrollHistoryRepo:
    def latest_event(self):
        return {
            ...
            "capitalCommitId": "commit-shell-1",
        }
...
class _PrimeStateRepo:
    def latest(self, state_type: str):
        return {
            ...
            "state_type": state_type,
            "payload": {"updatedTsMs": 1_700_000_000_100, "capitalCommitId": "commit-shell-1"},
        }
...
class _CapitalEventRepo:
    def latest_event(self, domain: str):
        mapping = {
            "bankroll": {"ts_ms": 1_700_000_000_080, "event_type": "bankroll", "payload": {"capitalCommitId": "commit-shell-1", "state": {"updated_ts_ms": 1_700_000_000_080}}},
            "treasury": {"ts_ms": 1_700_000_000_090, "event_type": "treasury", "payload": {"capitalCommitId": "commit-shell-1", "capital_engine": {"updated_ts_ms": 1_700_000_000_090, "deployable_bankroll_wei": 25 * 10**18, "estimated_capital_wei": 30 * 10**18, "drawdown_buffer_wei": 5 * 10**18, "family_allocations_wei": {"flashloan_atomic": 12 * 10**18}}}},
            "ledger": {"ts_ms": 1_700_000_000_200, "event_type": "ledger", "payload": {"capitalCommitId": "commit-shell-1", "tx_type": "receipt_settlement"}},
            "receipt": {"ts_ms": 1_700_000_000_500, "event_type": "receipt", "payload": {"capitalCommitId": "commit-shell-1", "ts_ms": 1_700_000_000_500}},
            "internal_prime": {"ts_ms": 1_700_000_000_100, "event_type": "internal_prime", "payload": {"capitalCommitId": "commit-shell-1", "updatedTsMs": 1_700_000_000_100}},
        }
...
def test_capital_truth_service_shell_helper_matches_service_summary(monkeypatch) -> None:
    ...
    expected_reason = str(shell.initial_bundle.reconciliation_payload.get("status_reason_code") or "ok")
    assert runtime._capital_recovery_repo.calls == [
        {
            "component": "capital_truth",
            "degraded": bool(str(shell.initial_bundle.reconciliation_payload.get("status") or "ok") != "ok"),
            "ts_ms": 1_700_000_000_500,
            "reason_code": expected_reason,
        }
    ]
    assert shell.finalization.recovery_history["last_reason_code"] == expected_reason
    assert shell.truth["canonical"] is True
    assert shell.truth["summaryContract"]["readModel"] == "capital_truth_projection_v1"
    assert summarize_capital_truth(service=service, runtime=runtime) == shell.truth
    assert service.summary(runtime) == shell.truth
