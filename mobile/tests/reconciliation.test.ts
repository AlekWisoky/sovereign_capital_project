...
import { Reconciler, classifyReconciliationFreshness } from '../src/api/reconciliation';
import { externalWalletTransactionFromPrepared, reconcileSubmittedExternalWalletTransaction, validatePreparedExternalWalletTransaction } from '../src/api/offRampExternalWallet';
...
test('realtime does not make canonical truth fresh when authoritative polling is stale', () => {
  const reconciler = makeReconciler();
  reconciler.acceptAuthoritative({ value: 1 }, 100, 1_000);
  reconciler.acceptRealtime({ value: 2 }, 200, 15_000);
...
test('errors do not erase the last known good canonical value', () => {
  ...
  assert.deepEqual(reconciler.snapshot(2_000).value, { value: 7 });
});
...
test('external-wallet prepared transaction binds the connected wallet when backend prepare is sender-agnostic', () => {
  ...
  const tx = externalWalletTransactionFromPrepared({ ok: true, from_address: null, requested_from_address: null, tx: { to: '0x2222222222222222222222222222222222222222', data: '0x1234', value: '0x0', chainId: 1 } });
  assert.deepEqual(tx, { from: address, to: '0x2222222222222222222222222222222222222222', data: '0x1234', value: '0x0', chainId: '0x1' });
...
test('external-wallet validation rejects a prepared sender different from the connected wallet', async () => {
  ...
  const result = await validatePreparedExternalWalletTransaction({ ok: true, from_address: prepared, tx: { to: '0x2222222222222222222222222222222222222222', data: '0x1234', value: '0x0', chainId: 1 } });
