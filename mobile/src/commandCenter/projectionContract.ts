import type { AlertItem, ProjectionCompatibility, SummaryReadContract } from "./types";
...
export function normalizeSummaryContract(value: unknown): SummaryReadContract | undefined {
  ...
  return {
    ...
    capitalContractVersion: pickString(record.capitalContractVersion ?? record.capital_contract_version),
...
export function evaluateProjectionCompatibility(
  ...
  if (!summaryContract) {
    ...
    if (summaryContract.contractVersion && summaryContract.contractVersion !== CANONICAL_SUMMARY_CONTRACT_VERSION) {
  ...
  if (expectation.fallbackUsed) reasonCodes.push("legacy_projection_fallback_used");
...
export function mergeProjectionCompatibility(
  ...
  const normalized = Object.fromEntries(Object.entries(sources).filter(([, value]) => Boolean(value))) as Record<string, ProjectionCompatibility>;
  if (!Object.keys(normalized).length) return undefined;
  const reasonCodes = Object.entries(normalized).flatMap(([name, value]) => value.reasonCodes.map((code) => `${name}:${code}`));
  ...
  return { status: degraded ? "degraded" : warning ? "warning" : "canonical", reasonCodes, sources: normalized };
}
